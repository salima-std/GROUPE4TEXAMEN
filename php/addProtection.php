<?php
	...
	if ('Enregistrer'==$_POST['btnEnregistrerProt']) 
	{
		...
		try {
			...
			$st->execute(array('designation'=> $Protection->getDesignationProt(),'agent'=> $_SESSION['ID_AGENT']));
	...
	elseif ('Modifier'==$_POST['btnEnregistrerProt']) 
	{
		...
		try {
			...
			$st->execute(array('code' => $Protection->getIdProt(),'designation'=> $Protection->getDesignationProt(),'agent'=> $_SESSION['ID_AGENT']));
			header('Location: ../protection');
			$st->closeCursor();
		} catch (Exception $e) {
			die('Erreur: '.$e->getMessage());
