<?php
	...
	if ('Enregistrer'==$_POST['btnEnregistrerQR']) 
	{
		...
		try {
			...
			$st->execute(array('QUESTION'=> $RepQuest->getQuestionRQ(),'REPONSE'=> $RepQuest->getReponseRQ(),'AGENT_ID'=>$_SESSION['ID_AGENT']));
	...
	elseif ('Modifier'==$_POST['btnEnregistrerQR']) 
	{
		...
		try {
			...
			$st->execute(array('CODE'=> $RepQuest->getIdRQ(),'QUESTION'=> $RepQuest->getQuestionRQ(),'REPONSE'=> $RepQuest->getReponseRQ(),'AGENT_ID'=>$_SESSION['ID_AGENT']));
			header('Location: ../repQuest');
			$st->closeCursor();		
		} catch (Exception $e) {
			die('Ereur: '.$e->getMessage());
